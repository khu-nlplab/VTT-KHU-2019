...
class SequenceClassification(nn.Module):
    ...
    def __init__(self, dict_dim, embedding_dim, dropout_prob, num_labels, device):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=embedding_dim*24, nhead=8)
        encoder_norm = nn.LayerNorm(embedding_dim*24)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1, norm=encoder_norm)
    ...
    def forward(self, q_vec, d_vec, sd_vec, labels=None):
        ...
        linear_result = self.transformer_encoder(all_concat_input)
        result_output = self.result_Linear(linear_result) #[batch_size, 128, embedding_size*24]
