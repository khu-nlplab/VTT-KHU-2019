...
# from utils_as import (compute_metrics, convert_examples_to_features,
#                                        output_modes, processors)

from utils_as import (compute_metrics, convert_examples_to_features,
                                       output_modes, processors, write_predictions)
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForAnswerSelection, BertTokenizer),
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    processor = processors[task](args.num_labels)
...
def initialize(args):
    ...
    processor = processors[args.task_name](args.num_labels)
    ...
    return (model, tokenizer, global_step)

def inference(args, model, tokenizer, global_step):
...
def main():
    ...
    parser.add_argument('--initialize_model', type=bool, default=True,
                        help="initialize model and preprocessing.")
    parser.add_argument('--inference_model', type=bool, default=True,
                        help="inference model.")
    args = parser.parse_args()

    results = {}

    if args.initialize_model is True:
        model, tokenizer, global_step = initialize(args)

    if args.inference_model is True:
        result = inference(args, model, tokenizer, global_step)
        results.update(result)
